...
| 6.4b | **後續（2026-09-13）：三個 action 已升到 Node 24 版本。** `actions/checkout` v4.2.2 → v7.0.1（`3d3c42e5aac5ba805825da76410c181273ba90b1`）、`actions/setup-python` v5.6.0 → v7.0.0（`5fda3b95a4ea91299a34e894583c3862153e4b97`）、`actions/upload-artifact` v4.6.2 → v7.0.1（`043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`），兩個 workflow 共 11 處。升版前對照各 tag 的 `action.yml`：三者皆 `using: node24`；輸入只有新增（checkout `allow-unsafe-pr-checkout` 預設 false、setup-python `pip-version`、upload-artifact `archive` 預設 true），無移除；checkout v7 預設拒絕在 `pull_request_target`／`workflow_run` 下取出 fork PR（本 repo 不用這兩個觸發），v6 把憑證改存 `$RUNNER_TEMP`（本 repo 一律 `persist-credentials: false`）；Node 24 需 runner ≥ v2.327.1（GitHub-hosted 已符合）。gitleaks-action 已於 6.1b 移除，不再需要升版。SHA 以 `git ls-remote --tags` 取得並由本盤點腳本核對 tag 註解。 | 已修正 | `.github/workflows/mara-review.yml`、`.github/workflows/governance.yml` |
| 6.4c | **後續（2026-09-13）：兩個 workflow 加上 `concurrency:`，zizmor auditor persona 歸零。** `mara-review.yml`：group `mara-review-<PR 號或 ref>`，`pull_request` 時新 push 取消被取代的 run，push 到 main 時排隊不取消（否則較早合併的 human-queue job 會被後一個 push 跳過）；`governance.yml`：group `governance-check`、永不取消（手動 dispatch 排在每週 run 之後，追蹤 issue 一定拿到每次啟動的結果）。同時給 `governance.yml` 的 `issues: write` 補上用途註解（auditor persona 的 `undocumented-permissions`）。表達式只出現在 `concurrency:`，`run:` 內仍無 `${{ }}`。 | 已修正 | `.github/workflows/mara-review.yml`、`.github/workflows/governance.yml` |
| 6.4d | **後續（2026-09-14）：repo 改為 public，L0 的 SARIF 接進 GitHub Code Scanning；zizmor-action 由 lock 內的 CLI 取代；新增 Scorecard workflow 與 Dependabot。** `mara-review.yml` 新 job `code-scanning`（`needs: deterministic-tools`、`security-events: write`、不執行 repo 程式、fork 與 Dependabot 的 PR 跳過）用 `actions/download-artifact` v8.0.1（`3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`）取回 `l0-sarif`，`scripts/code_scanning_prep.py` 整理後以 `github/codeql-action/upload-sarif` v4.38.0（`b96794f015dfd88f77b49b1c93e0fa7110f94c63`）上傳五份對本 repo 的掃描；`zizmorcore/zizmor-action` 移除，改為 `scripts/zizmor_ci.py` 執行 lock 內驗過 attestation 的 zizmor（線上稽核帶 token；`--format sarif` 時 zizmor 有 finding 也 exit 0，所以由腳本讀 SARIF 判定）。`scorecard.yml`（push 到 main、每週二、手動；`permissions: {}`、job 只有 `contents: read` 與 `security-events: write`，不用 `id-token`）以 lock 內 SLSA 驗證的 scorecard CLI 5.5.0 跑，`scripts/scorecard_ci.py` 轉 SARIF 並依 `tools/scorecard-policy.yaml` 把關。`.github/dependabot.yml` 只管 github-actions 生態系。三個 workflow 對 zizmor 的 default、auditor、pedantic persona 皆 0 finding，本盤點腳本評為 None，`uses:` 全為 SHA。 | 已修正 | `.github/workflows/mara-review.yml`、`.github/workflows/scorecard.yml`、`.github/dependabot.yml`、`scripts/zizmor_ci.py`、`scripts/code_scanning_prep.py`、`scripts/scorecard_ci.py`、`docs/tools-provenance.md` 3d |
| 6.4e | **後續（2026-09-17）：Scorecard 第一次真實分數，以及只跑了 9 個 check 的原因。** 合併 PR #21 後 main 上的 `scorecard.yml` run 1：總分 5.6；Binary-Artifacts、Dangerous-Workflow、Token-Permissions、CI-Tests 10；Pinned-Dependencies 7（tests job 的 `pip install -e .` 無 hash）；Code-Review 0（PR 都由作者合併）；License、Security-Policy 0（無 `LICENSE`、`SECURITY.md`）；Vulnerabilities 0（11 個 advisory 全是 `fixtures/vuln-sample` 與 `calib/samples` 的種子套件，Scorecard 不能排除目錄）。只回報 9 個 check 的原因在 scorecard 5.5.0 的 `pkg/scorecard/scorecard.go`：`--commit` 不是 `HEAD` 時要求 check 支援 `CommitBased`，Branch-Protection、Maintained、SAST、Dependency-Update-Tool、Signed-Releases 等以 `registerCheck(..., nil)` 註冊者被略過，而 workflow 傳了 `--commit "$SHA"`。修正：workflow 不再傳 `--commit`（push run 的 HEAD 就是該 commit），`scripts/scorecard_ci.py` 只在指定非 HEAD 時才傳、並把 scorecard 沒回報的 check 印成 `not run by scorecard`；`tools/scorecard-policy.yaml` 與 3d 節的文件改正（Branch-Protection 之前是「被略過」不是「-1」）。另：本盤點腳本的 `SKIP_DIRS` 加入 `.mara-tools`，不再把 semgrep-rules checkout 自帶的 11 個 workflow 當成本 repo 的 workflow 盤點。 | 已修正 | `.github/workflows/scorecard.yml`、`scripts/scorecard_ci.py`、`scripts/actions_inventory.py`、`tools/scorecard-policy.yaml`、`docs/tools-provenance.md` 3d |
| 6.4f | **後續（2026-09-17）：三個 job 的 `pip install -e .` 改為 hash 鎖定閉包。** Scorecard Pinned-Dependencies 7 分的來源是 tests、human-queue、governance 三個 job 的 `pip install -e ".[dev]"`／`-e .`（依賴解析在 CI 當下決定，無 hash）。改為先 `pip install --require-hashes --only-binary=:all: -r tools/dev-requirements.txt`（`pyproject.toml` 依賴加 dev extra 的完整閉包，37 個 wheel，由 `scripts/relock_requirements.py --from-pyproject dev` 以 `pip install --dry-run --report` 解析、SHA-256 取自 PyPI 索引），再 `pip install --no-deps -e .` 只裝本 repo 程式（Scorecard 對 `--no-deps -e <本地路徑>` 視為已固定）。`tools/model-eval-requirements.txt` 同樣改為 garak 0.17.0 的 194 個 distribution 閉包（兩個 sdist）。新測試 `tests/test_relock_requirements.py` 會對每個 workflow 檢查沒有任何不帶 `--require-hashes` 的 `pip install`。osv 閉包步驟加入 dev 閉包。 | 已修正 | `.github/workflows/mara-review.yml`、`.github/workflows/governance.yml`、`scripts/relock_requirements.py`、`tools/dev-requirements.txt`、`tools/model-eval-requirements.in`／`.txt` |
| 6.4g | **後續（2026-09-17）：合併 PR #24 後的 Scorecard 分數，Pinned-Dependencies 8 的天花板與 policy 強制。** main 的 `scorecard.yml` run 4：總分 4.4；Pinned-Dependencies 8（前 7）、SAST 2（前 1）、Dependency-Update-Tool 10，其餘不變。8 不是預期的 10：以 lock 內的 scorecard 5.5.0 對 main 的乾淨匯出跑 `scorecard --local . --checks=Pinned-Dependencies` 重現同分，五條警告全在兩個刻意種下的 Dockerfile：`fixtures/vuln-sample/Dockerfile`（第 1 行 `FROM python:latest`、第 2 行 `curl | sh`、第 4 行無 hash 的 `pip install -r`）與 `calib/samples/s1-fastapi-inventory/Dockerfile`（第 1 行 `FROM python:latest`、第 2 行 `curl | bash`）。本 repo 自己的 workflow 為 18/18 GitHub-owned action 固定、9 條 pip 指令 8 條固定（未固定那條就是 fixture Dockerfile 的）。原因在 scorecard 的檔案比對：Dockerfile 的樣式 `*Dockerfile*` 只比檔名、全 repo 適用且不能排除目錄；workflow 的樣式 `.github/workflows/*` 含 `/`，只比對根目錄，所以 fixture 內的 workflow 不計。計分（`checks/evaluation/pinned_dependencies.go` 的加權比例分）：10 × (18＋0＋0＋8) ÷ (18＋2＋2＋9) ≈ 8.4 → 8。結論：種子材料在，這項天花板就是 8，退到 7 才是 workflow 真的退步。`tools/scorecard-policy.yaml` 因此新增 `Pinned-Dependencies: 8` 為第四個強制項並記錄推導；`tests/test_scorecard_ci.py` 新增檢查：repo 內的 Dockerfile 只能在 `fixtures/`、`calib/samples/` 之下，真實 Dockerfile 若未以 digest 固定映像、未以 hash 安裝，會在 PR 階段被抓到而不是合併後才紅。 | 已修正 | `tools/scorecard-policy.yaml`、`tests/test_scorecard_ci.py`、`README.md`、`docs/tools-provenance.md` 3d |
| 6.4h | **後續（2026-09-17）：SECURITY.md 合併後 Security-Policy 10 分，與 Dependency-Update-Tool 一起加入 policy 強制。** 合併 PR #26 後 main 的 `scorecard.yml` run 6：總分 5.1；Security-Policy 由 0 升到 10（`SECURITY.md`：private vulnerability reporting 連結、範圍、處理流程、揭露用語，四個 probe 全中）；Dependency-Update-Tool 在 run 4、5、6 皆 10（`.github/dependabot.yml`）；SAST 4；Pinned-Dependencies 8（policy ≥ 8 達標）。`tools/scorecard-policy.yaml` 因此把 Security-Policy 與 Dependency-Update-Tool 各以 10 加入 `enforce`，強制項共六個：SECURITY.md 被刪或退化、Dependabot 設定失效時，main 上的 scorecard job 會紅。 | 已修正 | `tools/scorecard-policy.yaml`、`tests/test_scorecard_ci.py`、`README.md`、`docs/tools-provenance.md` 3d |
